...
public class OpenNISkeleton : MonoBehaviour 
{
	...
	public void Start()
	{
		...
		shouldRotateJoint = new bool[jointCount];
		for( int idx = 0; idx < shouldRotateJoint.Length; idx++ ) shouldRotateJoint[idx] = true;
	...
	public void UpdateJoint(SkeletonJoint joint, SkeletonJointTransformation skelTrans)
	{
        ...
        if (UpdateOrientation && skelTrans.Orientation.Confidence > 0.5 && shouldRotateJoint[(int)joint])
        {
			...
			transforms[(int)joint].rotation = Quaternion.Slerp(transforms[(int)joint].rotation, newRotation, (Time.deltaTime / Time.timeScale) * RotationDamping);
        }
